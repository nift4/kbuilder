...
releaseId=$(curl -O- --user "$GH_USER:$GH_TOKEN" -X POST https://api.github.com/repos/${GH_REPO_USER}/${GH_REPO}/releases -d "{\"tag_name\": \"$(date +%4Y%m%d%H%M)\",\"target_commitish\": \"$Branch\",\"name\": \"$(date +%4Y%m%d%H%M) Kernel\",\"body\": \"Automatically built.\",\"draft\": false,\"prerelease\": false}" | python -c 'import json,sys;print(json.load(sys.stdin)["id"])')
